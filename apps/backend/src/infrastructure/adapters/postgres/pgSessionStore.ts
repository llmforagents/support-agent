...
import { Ok, Err, type Result, type AppError, SessionId, MessageId, VisitorId, UsdCents } from '@support/shared'
...
type MRow = { id: string; session_id: string; role: Message['role']; content: string; cost_cents: number; created_at: Date }
...
export class PgSessionStore implements SessionStorePort {
  ...
  async appendMessageWithId(input: { id: MessageId; sessionId: SessionId; role: Message['role']; content: string; costCents: UsdCents }): Promise<Result<Message, AppError>> {
    const client = await this.pool.connect()
    try {
      await client.query('BEGIN')
      const r = await client.query<MRow>(
        `INSERT INTO messages (id, session_id, role, content, cost_cents) VALUES ($1, $2, $3, $4, $5)
         RETURNING id, session_id, role, content, cost_cents, created_at`,
        [input.id, input.sessionId, input.role, input.content, input.costCents],
      )
      await client.query(
  ...
  async listMessages(id: SessionId, opts: { limit: number; afterId?: MessageId }): Promise<Result<readonly Message[], AppError>> {
    try {
      const baseSql = `SELECT id, session_id, role, content, cost_cents, created_at FROM messages WHERE session_id = $1`
      let r
      if (opts.afterId) {
