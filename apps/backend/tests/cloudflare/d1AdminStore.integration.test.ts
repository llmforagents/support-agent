...
describe('D1AdminStore @integration', () => {
  beforeEach(async () => {
    await runD1Migrations(env.DB)
    // vitest-pool-workers isolates storage per test file, not per test —
    // clear the tables between tests so inserts don't collide. ON DELETE
    // CASCADE on admin_sessions sweeps sessions too.
    await env.DB.prepare('DELETE FROM admins').run()
  })

  it('countAdmins on empty db = 0', async () => {
    const store = new D1AdminStore(env.DB)
